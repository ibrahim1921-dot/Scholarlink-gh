...
export const ScholarshipApplicationList: React.FC = () => {
  const {
  ...
  return (
    <Card className="shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between pb-6">
        <CardTitle className="text-2xl text-primary font-bold">Scholarship Applications</CardTitle>
              ...
              {isLoading ? (
                ...
                data?.data.map((app: any) => (
                  <TableRow key={app.id}>
                    ...
                    <TableCell>{app.scholarshipName || "N/A"}</TableCell>
                    <TableCell>{app.scholarshipProvider || "N/A"}</TableCell>
                    <TableCell>
                      {app.applicationMode === "ASSISTED" ? (
      ...
      <AlertDialog open={!!actionApp} onOpenChange={(open) => !open && setActionApp(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Change the application status for {actionApp?.scholarshipName} from {actionApp?.status?.replace("_", " ")} to {actionType?.replace("_", " ")}?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
